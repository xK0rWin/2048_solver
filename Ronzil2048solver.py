...
class Game2048:
    def __init__(self, master):
        ...
        self.depth = 4  # Expectimax search depth
        self.simulations_per_move = 1  # single Expectimax path, can be adjusted

        # Precompute log2 for speed
        self.log2_table = {0:0}
    ...
    def create_ui(self):
        ...
        for r in range(SIZE):
            for c in range(SIZE):
                lbl = tk.Label(frame, text='', width=6, height=3, font=('Arial', 18),
    ...
    def place_random_tile(self, board):
        ...
        board[r,c] = 2 if random.random()<0.75 else 4
    ...
    def evaluate_board(self, board):
        ...
        for r in range(SIZE):
            for c in range(SIZE):
                val = board[r,c]
                if val==0: continue
                log_val = self.log2_table[val]
                # Smoothness
                for dr,dc in [(1,0),(0,1)]:
                    nr,nc = r+dr, c+dc
                    if 0<=nr<SIZE and 0<=nc<SIZE and board[nr,nc]!=0:
                        smoothness -= abs(log_val - self.log2_table[board[nr,nc]])
                # Snake pattern
                monotonicity += val*snake_weight[r][c]
        corner_bonus = max_tile if max_tile in [board[0,0],board[0,3],board[3,0],board[3,3]] else 0
        return empty_cells*150 + smoothness + monotonicity + corner_bonus*2
    ...
    def find_best_move(self, board):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for move in moves:
                new_board = self.move_board(board, move)
                if np.array_equal(new_board, board): continue
                futures.append(executor.submit(lambda m,b: (m, self.expectimax(b,self.depth-1,False)), move, new_board))
    ...
    def step_automation(self):
        ...
        if best != "None":
            self.make_move(best)
        self.master.after(50, self.step_automation)
